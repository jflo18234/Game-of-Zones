...
async def sync_current_matchups():
    """
    Keep the Matchups channel on the current Sleeper week and edit the bot's
    existing matchup posts as scores change.
    """
    ...
    if channel is None:
        print(f"Matchups: channel not found ({MATCHUPS_CHANNEL_ID}).")
        return False, week, 0

    # Only manage posts created by this matchup updater. Older recap/other
    # messages in the channel are left alone.
    existing = []
    try:
        async for message in channel.history(limit=100):
            if (
                ...
                and (
                    "FANTASY BATTLES" in message.content
                    or "FANTASY BATTLES — CONTINUED" in message.content
                )
            ):
                existing.append(message)
    except (discord.Forbidden, discord.HTTPException) as e:
        print(f"Matchups: could not read channel history: {e}")
        return False, week, 0

    # Discord history is newest-first; edit in chronological order.
    existing.reverse()

    for index, content in enumerate(desired_messages):
        # Give continuation chunks an identifiable header so they can be found
        # again after a Railway restart.
        ...
        if index < len(existing):
            ...
            existing.append(await channel.send(content))

    # If a new week/content needs fewer chunks, remove only obsolete matchup
    # chunks from this updater.
    ...
    print(
        ...
        f"({len(desired_messages)} permanent message(s))."
    )
