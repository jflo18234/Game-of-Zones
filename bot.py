...
async def sync_current_matchups():
    """
    Refresh the current Sleeper manager-vs-manager scoreboard.
    Sending a new board does NOT require Read Message History permission.
    """
    state = get_json("https://api.sleeper.app/v1/state/nfl")
    week = int(state.get("week", 1) or 1)
    desired_messages = await get_fantasy_matchup_message(week)

    # get_channel can return None immediately after startup/cache changes.
    # Fall back to Discord's API before declaring the channel unavailable.
    ...
    if channel is None:
        try:
            ...
            print(
                f"Matchups: cannot resolve channel {MATCHUPS_CHANNEL_ID}: "
                f"{type(e).__name__}: {e}"
            )
            return False, week, 0
    ...
    try:
        ...
        # This used to abort the entire sync, which meant NOTHING was posted
        # when the bot lacked Read Message History. It can still send a fresh
        # board, so continue instead of returning.
        history_available = False
        print(
            "Matchups: history unavailable; will send a fresh board instead: "
            f"{type(e).__name__}: {e}"
        )
    ...
    try:
        for index, raw_content in enumerate(desired_messages):
            content = raw_content
            if index > 0 and "FANTASY BATTLES" not in content:
                content = (
                    f"⚔️ **GAME OF ZONES — WEEK {week} "
                    f"FANTASY BATTLES — CONTINUED** ⚔️\n\n"
                    + content
                )

            if history_available and index < len(existing):
                if existing[index].content != content:
                    await existing[index].edit(content=content)
            else:
                await channel.send(content)

        if history_available:
            for obsolete in existing[len(desired_messages):]:
                try:
        ...
        print(
            f"Matchups: Discord send/edit failed in channel "
            f"{MATCHUPS_CHANNEL_ID}: {type(e).__name__}: {e}"
        )
        return False, week, 0

    print(
        f"Matchups: Week {week} synced; "
        f"Sleeper chunks={len(desired_messages)}, "
        f"history_available={history_available}."
    )
    return True, week, len(desired_messages)


@bot.command()
async def matchuptest(ctx):
    """Show exactly where the current matchup pipeline succeeds/fails."""
    try:
        state = get_json("https://api.sleeper.app/v1/state/nfl")
        week = int(state.get("week", 1) or 1)
        raw_matchups = get_json(
        ...
        for row in raw_matchups:
            ...
            if mid is not None:
                paired.setdefault(mid, []).append(row)
        paired_count = sum(1 for teams in paired.values() if len(teams) >= 2)

        cached_channel = bot.get_channel(MATCHUPS_CHANNEL_ID)
        channel_resolved = cached_channel is not None
        if cached_channel is None:
            try:
                await bot.fetch_channel(MATCHUPS_CHANNEL_ID)
        ...
        await ctx.send(
            ...
            f"Week: {week}\n"
            f"Sleeper matchup rows: {len(raw_matchups)}\n"
            f"Paired manager matchups: {paired_count}\n"
            f"League rosters: {len(rosters)}\n"
            f"League users: {len(users)}\n"
            f"Matchups channel resolved: {channel_resolved}\n"
            f"Messages/chunks built: {chunks}\n"
...
async def matchup_score_checker():
    """Refresh manager-vs-manager Sleeper scores every five minutes."""
    try:
