...
def build_weekly_stats_chunks_from_sleeper(current_week, rosters_data, user_names, matchups):
    """Build the clean weekly feed directly from Sleeper's live matchup payload.

    This intentionally does not depend on week_player_stats.json or ESPN.
    Any rostered player for whom Sleeper currently reports a non-zero
    players_points value is included. The feed is rebuilt from scratch each
    refresh, so stale local cache data cannot leave the channel empty.
    """
    roster_to_team = {}
    roster_to_players = {}
    ...
    for roster in rosters_data:
        ...
        owner_id = roster.get("owner_id")
        roster_to_team[roster_id] = user_names.get(
            owner_id, f"House {roster.get('roster_id', '?')}"
        )
        roster_to_players[roster_id] = {
            str(pid) for pid in (roster.get("players") or []) if pid and pid != "0"
        }

    matchup_by_roster = {
        str(m.get("roster_id")): m for m in (matchups or [])
    ...
    for roster in sorted(rosters_data, key=lambda r: r.get("roster_id") or 0):
        roster_id = str(roster.get("roster_id"))
        team_name = roster_to_team.get(roster_id, "Unknown House")
        matchup = matchup_by_roster.get(roster_id, {}) or {}
        player_points = matchup.get("players_points") or {}

        rows = []
        for pid, raw_points in player_points.items():
            pid = str(pid)
            if pid not in roster_to_players.get(roster_id, set()):
                continue
            try:
            ...
            # For this data-feed channel, Sleeper is the source of truth.
            # Non-zero values mean the player has accumulated fantasy stats.
            if abs(points) <= 0.000001:
                continue

            info = players_cache.get(pid, {}) or {}
            name = get_player_name(pid)
            position = info.get("position", "")
            rows.append((position, name, points))
            included_count += 1
        ...
        if rows:
            for position, name, points in rows:
                ...
                lines.append(label)
                lines.append(f"Fantasy Points: {points:.2f}")
        else:
            lines.append("No player scores recorded by Sleeper yet.")
    ...
    for block in blocks:
        ...
        chunks.append(current.rstrip())
        current = f"**WEEK {current_week} - NFL PLAYER STATS (CONTINUED)**\n\n" + addition

    if current.strip():
        chunks.append(current.rstrip())

    print(
        f"Direct Sleeper weekly feed for Week {current_week}: "
        f"{len(matchups or [])} matchup rows, {included_count} scored players."
    )
    return chunks
