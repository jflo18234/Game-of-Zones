...
async def player_stats_checker():
    try:
        ...
        # Recover stats that an older bot version may already have posted in the
        # detailed Stats channel but failed to persist into the cumulative cache.
        # This makes the weekly feed immediately inherit those completed results.
        await backfill_weekly_stats_from_detailed_channel(
            current_week, week_data
        )

        # Keep the dedicated no-emoji weekly data feed present and current even
        # after a restart, using both persisted and recovered completed stats.
