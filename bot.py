...
# Ensure persistence storage exists before any cache/tracker is loaded or saved.
# Railway may start with an empty mounted volume at /data.
try:
    os.makedirs(DATA_DIR, exist_ok=True)
except OSError as e:
    print(f"WARNING: Could not create DATA_DIR {DATA_DIR!r}: {e}")


def ensure_data_dir():
    """Ensure the persistence directory exists before a file write."""
    os.makedirs(DATA_DIR, exist_ok=True)
...
# Current-process IDs for the permanent matchup board.
MATCHUP_MESSAGE_IDS = []
LAST_MATCHUP_SYNC_ERROR = "None"

central = ZoneInfo("America/Chicago")
BOT_PROCESS_START_MS = int(datetime.now(tz=central).timestamp() * 1000)
...
def load_week_player_stats():
    if os.path.exists(WEEK_PLAYER_STATS_FILE):
        try:
            ...
            print(f"Could not load weekly player stats cache: {e}")
    return {}
...
def get_json(url):
    # A browser-like User-Agent avoids providers rejecting Python's default
    # urllib agent, and the timeout prevents one stalled API request from
    # freezing a scheduled task indefinitely on Railway.
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": (
                "Mozilla/5.0 (compatible; GameOfZonesBot/1.0; "
                "+https://discord.com)"
            ),
...
def get_week_mvp(week):
    """Return the highest-scoring starter using Sleeper's weekly player stats."""
    rosters = get_json(
    ...
    nfl_state = get_json("https://api.sleeper.app/v1/state/nfl")
    season = int(nfl_state.get("season", datetime.now(central).year) or datetime.now(central).year)
    scoring_settings = league.get("scoring_settings", {}) if isinstance(league, dict) else {}
    ...
    for roster in rosters:
        team_name = user_names.get(roster.get("owner_id"), "Unknown House")
        for raw_pid in roster.get("starters", []) or []:
            pid = str(raw_pid)
            if pid and pid != "0":
                starter_owner[pid] = team_name

    stats_by_pid = {}
    for row in get_sleeper_week_player_stats(season, int(week)):
        pid = sleeper_stat_row_player_id(row)
        if not pid or pid not in starter_owner:
            continue
        stats = sleeper_stat_row_stats(row)
        if stats:
            stats_by_pid[pid] = stats

    best = None
    for pid, team_name in starter_owner.items():
        stats = stats_by_pid.get(pid)
        if not stats:
            continue
        pts = calculate_league_fantasy_points(stats, scoring_settings)
...
async def sync_weekly_stats_feed(current_week, rosters_data, user_names, week_data=None):
    """Edit the clean weekly feed from cached Sleeper weekly player stats.

    ``week_data`` is populated from Sleeper's per-player stats service and
    mapped directly to fantasy rosters by Sleeper player ID.
    """
    channel = bot.get_channel(ROSTER_STATS_CHANNEL_ID)
    if channel is None:
        try:
            channel = await bot.fetch_channel(ROSTER_STATS_CHANNEL_ID)
        except discord.HTTPException as e:
            print(
                f"Could not find cumulative NFL player stats channel "
                f"{ROSTER_STATS_CHANNEL_ID}: {e}"
            )
            return False

    if week_data is None:
        week_data = week_player_stats.setdefault(str(current_week), {"teams": {}})

    chunks = build_weekly_stats_chunks(
        str(current_week), rosters_data, user_names, week_data
    )

    teams_data = week_data.get("teams", {}) or {}
    player_count = sum(len(players or {}) for players in teams_data.values())
    print(
        f"Weekly cumulative stats feed for Week {current_week}: "
        f"{player_count} rostered player(s) cached from Sleeper weekly stats."
    )
    ...
    # If persistent IDs were lost, rediscover this week's existing bot posts
    # from the channel so a restart does not create duplicate weekly feeds.
    if not ids:
        try:
            async for msg in channel.history(limit=100):
                if bot.user is not None and msg.author.id != bot.user.id:
                    continue
                if f"WEEK {current_week} - NFL PLAYER STATS" in (msg.content or ""):
                    ids.append(str(msg.id))
            ids.reverse()
        except (discord.Forbidden, discord.HTTPException) as e:
            print(f"Could not rediscover weekly stats posts: {e}")
    ...
    for index, content in enumerate(chunks):
        ...
        if index < len(ids):
            try:
                ...
                print(f"Could not fetch weekly stats message {ids[index]}: {e}")
                return False
    ...
    print(
        f"Synced Week {current_week} completed player stats across "
...
def get_recent_nfl_games(days_back=3, week=None, season=None):
    """Return ESPN NFL events, preferring the whole fantasy/NFL week.

    ESPN's week scoreboard is much more reliable for a cumulative fantasy
    feed than asking for individual calendar dates.  Date queries remain as a
    fallback so the bot can still recover if the week endpoint is unavailable.
    """
    events_by_id = {}
    ...
    # Primary source: the complete regular-season week.  This captures the
    # Thursday-through-Monday slate in one response and survives restarts.
    if week is not None:
        week_url = (
            "https://site.api.espn.com/apis/site/v2/sports/football/nfl/"
            f"scoreboard?dates={int(season)}&seasontype=2&week={int(week)}&limit=100"
        )
        try:
            ...
            print(
                f"ESPN week scoreboard: season={season}, week={week}, "
                f"events={len(events_by_id)}"
            )
        except Exception as e:
            print(f"ESPN week scoreboard error for Week {week}: {type(e).__name__}: {e}")

    # Fallback/backfill: explicit calendar dates.  Only needed when the week
    # endpoint returned nothing or for extra resilience around season edges.
    if not events_by_id:
        for days_ago in range(days_back + 1):
            ...
            url = (
                "https://site.api.espn.com/apis/site/v2/"
                f"sports/football/nfl/scoreboard?dates={date_string}&limit=100"
            )

            try:
                data = get_json(url)
                date_events = data.get("events", []) or []
                print(f"ESPN date scoreboard {date_string}: events={len(date_events)}")
                ...
                print(
                    f"NFL scoreboard error for {date_string}: "
                    f"{type(e).__name__}: {e}"
                )
...
def get_current_league_week():
    """Return the league's active matchup week, preferring league-specific leg."""
    league = get_json(f"https://api.sleeper.app/v1/league/{LEAGUE_ID}")
    settings = league.get("settings") or {}

    # For league matchups, Sleeper's league leg is the best source of truth.
    # Fall back to the global NFL state only if the league object lacks it.
    league_leg = settings.get("leg")
    if league_leg is not None:
        try:
            return int(league_leg)
...
def build_fantasy_matchup_messages(week):
    """Build current manager-vs-manager scores directly from Sleeper."""
    matchups = get_json(
    ...
    for row in matchups:
        ...
        if matchup_id is not None:
            groups.setdefault(matchup_id, []).append(row)

    paired = []
    for matchup_id, teams in groups.items():
        if len(teams) >= 2:
            # A standard Sleeper H2H matchup has two roster rows.
            paired.append((matchup_id, teams[0], teams[1]))

    paired.sort(key=lambda item: item[0])

    header = (
        f"⚔️ **GAME OF ZONES — WEEK {week} FANTASY BATTLES** ⚔️\n"
        f"*Live scores from Sleeper — refreshed automatically*\n\n"
    )

    if not paired:
        return [
            header
            + f"No paired fantasy matchups are currently available for Week {week}."
        ], len(matchups), 0

    chunks = []
    current = header

    for _, team1, team2 in paired:
        ...
        block = (
            f"{mark1} **{name1}** — {score1:.2f}\n"
            f"{mark2} **{name2}** — {score2:.2f}\n\n"
        )

        if len(current) + len(block) > 1900:
            chunks.append(current.rstrip())
            current = (
                f"⚔️ **WEEK {week} FANTASY BATTLES — CONTINUED** ⚔️\n\n"
                + block
            )
        else:
            current += block
    ...
    return chunks, len(matchups), len(paired)


async def resolve_matchups_channel():
    """Resolve the configured Matchups channel, with a safe name fallback."""
    channel = bot.get_channel(MATCHUPS_CHANNEL_ID)
    if channel is not None:
        return channel

    try:
        channel = await bot.fetch_channel(MATCHUPS_CHANNEL_ID)
        if channel is not None:
            return channel
    except (discord.NotFound, discord.Forbidden, discord.HTTPException) as e:
        print(
            f"Matchups: configured channel ID {MATCHUPS_CHANNEL_ID} "
            f"could not be fetched: {type(e).__name__}: {e}"
        )

    # If the saved ID ever changed, prefer an obvious matchup-named text
    # channel instead of silently doing nothing.
    for guild in bot.guilds:
        for candidate in guild.text_channels:
            name = candidate.name.lower()
            if "matchup" in name or "fantasy-battle" in name:
                print(
                    f"Matchups: using fallback channel "
                    f"#{candidate.name} ({candidate.id})."
                )
                return candidate
    return None
...
async def get_fantasy_matchup_message(week):
    """Compatibility wrapper used by older commands/features."""
    try:
        messages, _, _ = build_fantasy_matchup_messages(int(week))
        return messages
    except Exception as e:
        print(
            f"Fantasy matchup formatting error: "
            f"{type(e).__name__}: {e}"
        )
        return [
            f"⚠️ I couldn't retrieve Week {week} fantasy matchups from Sleeper."
        ]


async def sync_current_matchups():
    """
    Maintain one live current-week matchup board.

    During a process, known message IDs are edited directly. On a fresh
    deploy, the bot recovers its latest matchup-board messages from history
    when permitted; otherwise it sends one board and remembers those IDs for
    subsequent edits during that process.
    """
    global MATCHUP_MESSAGE_IDS, LAST_MATCHUP_SYNC_ERROR
    LAST_MATCHUP_SYNC_ERROR = "None"

    week = get_current_league_week()
    desired, raw_rows, paired_count = build_fantasy_matchup_messages(week)
    channel = await resolve_matchups_channel()

    if channel is None:
        LAST_MATCHUP_SYNC_ERROR = (
            f"No usable Discord channel found for ID {MATCHUPS_CHANNEL_ID}"
        )
        print(f"Matchups: {LAST_MATCHUP_SYNC_ERROR}.")
        return False, week, 0

    existing = []

    # First try the IDs already known in this running process.
    for message_id in list(MATCHUP_MESSAGE_IDS):
        try:
            existing.append(await channel.fetch_message(int(message_id)))
        except (discord.NotFound, discord.Forbidden, discord.HTTPException):
            existing = []
            MATCHUP_MESSAGE_IDS = []
            break

    # On a restart, recover only the newest current-board messages.
    if not existing:
        try:
            recovered = []
            async for message in channel.history(limit=75):
                if (
                    ...
                    and "FANTASY BATTLES" in message.content
                    and "FINAL FANTASY SCORES" not in message.content
                    and "NFL GAMES ARE LIVE" not in message.content
                ):
                    recovered.append(message)

            # Keep only the newest contiguous board-sized set. We do not edit
            # every historical Week 1/Week 2 matchup message in the channel.
            recovered.sort(key=lambda m: m.created_at)
            if recovered:
                existing = recovered[-len(desired):]
                MATCHUP_MESSAGE_IDS = [m.id for m in existing]
        except (discord.Forbidden, discord.HTTPException) as e:
            print(
                "Matchups: Read Message History unavailable. "
                "A fresh board will be created once for this process: "
                f"{type(e).__name__}: {e}"
            )

    try:
        active = []
        for index, content in enumerate(desired):
            if index < len(existing):
                ...
                active.append(await channel.send(content))

        # Delete only extra messages from the recovered/current board, never
        # unrelated recaps or historical matchup posts.
        for msg in existing[len(desired):]:
            try:
                await msg.delete()
            except (discord.NotFound, discord.Forbidden, discord.HTTPException):
                pass

        MATCHUP_MESSAGE_IDS = [m.id for m in active]
        ...
        print(
            f"Matchups: Discord send/edit failed in #{getattr(channel, 'name', '?')} "
            f"({getattr(channel, 'id', '?')}): {LAST_MATCHUP_SYNC_ERROR}"
        )
        return False, week, 0

    print(
        f"MATCHUPS SYNC OK: week={week}; Sleeper rows={raw_rows}; "
        f"paired matchups={paired_count}; Discord messages={len(desired)}; "
        f"channel=#{getattr(channel, 'name', '?')} ({getattr(channel, 'id', '?')})."
    )
    return True, week, len(desired)


@tasks.loop(minutes=2)
async def matchup_score_checker():
    """Refresh the current manager-vs-manager scoreboard every two minutes."""
    try:
        await sync_current_matchups()
    except Exception as e:
        print(
            f"Automatic matchup score update error: "
            f"{type(e).__name__}: {e}"
        )


@matchup_score_checker.before_loop
async def before_matchup_score_checker():
    await bot.wait_until_ready()


@bot.command()
async def matchuptest(ctx):
    """Run the exact production matchup refresh and report its health."""
    try:
        week = get_current_league_week()
        messages, rows, pairs = build_fantasy_matchup_messages(week)
        channel = await resolve_matchups_channel()
        synced, synced_week, chunks = await sync_current_matchups()

        if channel and ctx.guild:
            me = ctx.guild.me
            perms = channel.permissions_for(me) if me else None
            permission_text = (
                f"View channel: {getattr(perms, 'view_channel', False)}\n"
                f"Send messages: {getattr(perms, 'send_messages', False)}\n"
                f"Read message history: {getattr(perms, 'read_message_history', False)}\n"
                f"Manage messages: {getattr(perms, 'manage_messages', False)}"
            )
        else:
            permission_text = "Discord permissions: unavailable"

        await ctx.send(
            "**Matchups diagnostic complete.**\n"
            f"League matchup week: {week}\n"
            f"Sleeper matchup rows: {rows}\n"
            f"Paired manager matchups: {pairs}\n"
            f"Expected for 32 teams: 32 rows / 16 pairs\n"
            f"Matchups channel: "
            f"{'#' + channel.name + ' (' + str(channel.id) + ')' if channel else 'NOT FOUND'}\n"
            f"Messages/chunks built: {len(messages)}\n"
            f"Matchups channel synced: {synced}\n"
            f"{permission_text}\n"
            f"Exact sync error: {LAST_MATCHUP_SYNC_ERROR}"
        )
    except Exception as e:
        print(f"Matchups diagnostic failed: {type(e).__name__}: {e}")
        await ctx.send(
            f"Matchups diagnostic failed: {type(e).__name__}: {e}"
        )
...
async def scores(ctx, week: int = None):
    try:
        state = get_json("https://api.sleeper.app/v1/state/nfl")
        current_week = int(state.get("week", 1) or 1)

        # Current week uses the same permanent messages as the automatic task.
        if week is None or int(week) == current_week:
            synced, synced_week, _ = await sync_current_matchups()
            if synced:
                await ctx.send(
                    f"⚔️ Week {synced_week} matchups and scores refreshed."
                )
            return

        # Historical week requests are displayed only on demand and do not
        # replace the permanent current-week matchup board.
        messages = await get_fantasy_matchup_message(int(week))
        for message in messages:
            await ctx.send(message)

    except Exception as e:
        print(f"Scores error: {type(e).__name__}: {e}")
        await ctx.send(
...
async def on_ready():
    ...
    # Seed all transactions that already existed before this process started.
    # This prevents historical waiver/trade reposts after a Railway redeploy,
    # even if the mounted/local tracking files were lost.
    await seed_existing_transaction_trackers()
    ...
    # Keep the current manager-vs-manager matchup board current immediately
    # after a deploy/restart, then refresh it every five minutes.
    try:
        await sync_current_matchups()
    except Exception as e:
        print(
            f"Startup matchup sync error: "
            f"{type(e).__name__}: {e}"
        )

    if not matchup_score_checker.is_running():
        matchup_score_checker.start()

    if not weekly_championship_check.is_running():
        weekly_championship_check.start()

    # Run one player-stats refresh immediately on startup, then let the
    # 15-minute task keep it current. The processed-game ledger prevents
    # duplicate detailed posts, while the cumulative feed is safely edited.
    try:
        startup_stats = await refresh_player_stats_pipeline(post_detailed=True)
        print(
            "Startup player stats refresh complete: "
            f"week={startup_stats.get('week')}, "
            f"sleeper_rows={startup_stats.get('sleeper_stat_rows')}, "
            f"rostered_rows={startup_stats.get('rostered_stat_rows')}, "
            f"listed={startup_stats.get('players_with_stats')}, "
            f"synced={startup_stats.get('cumulative_synced')}"
        )
    except Exception as e:
        print(f"Startup player stats refresh error: {type(e).__name__}: {e}")
...
async def waiver_checker():
    try:
        ...
        for t in transactions:
            ...
            if created_ms and created_ms <= BOT_PROCESS_START_MS:
...
async def seed_existing_transaction_trackers():
    """Mark transactions that already exist at startup as seen.

    This prevents old waiver/free-agent/trade history from being announced
    again after a deploy even when Railway starts with an empty local volume.
    Transactions created after startup are still announced normally.
    """
    try:
        nfl_state = get_json("https://api.sleeper.app/v1/state/nfl")
        week = int(nfl_state.get("week", 1) or 1)
        transactions = get_transactions(week) or []
        ...
        for transaction in transactions:
            ...
            if transaction_type in ("waiver", "free_agent"):
                waiver_ids.add(transaction_id)
            elif transaction_type == "trade":
                trade_ids.add(transaction_id)

        reported_transactions.update(waiver_ids)
        reported_trades.update(trade_ids)
        save_id_set(REPORTED_TRANSACTIONS_FILE, reported_transactions)
        save_id_set(REPORTED_TRADES_FILE, reported_trades)
        print(
            "TRANSACTION STARTUP SEED: "
            f"Week {week}; waivers/free agents={len(waiver_ids)}; "
            f"trades={len(trade_ids)}. Existing moves will not be reposted."
        )
        return True
    except Exception as e:
        print(f"Transaction startup seed error: {type(e).__name__}: {e}")
        return False
...
async def sync_all_roster_posts():
    ...
    # Railway/container restarts can lose the local roster_message_ids.json file.
    # If that happens, rediscover the bot's existing grouped roster posts directly
    # from Discord instead of creating another full set of duplicate rosters.
    if not roster_message_ids:
        discovered = []
        try:
            async for msg in channel.history(limit=100):
                if bot.user is not None and msg.author.id != bot.user.id:
                ...
                if "**Starters:**" in content and "**Bench:**" in content:
                    discovered.append(str(msg.id))
            discovered.reverse()
            if discovered:
                roster_message_ids[:] = discovered
                save_roster_message_ids(roster_message_ids)
                print(f"Rediscovered {len(discovered)} existing roster post(s) from Discord.")
        except (discord.Forbidden, discord.HTTPException) as e:
            print(f"Could not rediscover existing roster posts: {e}")
...
SLEEPER_STAT_GROUPS = [
    ("Passing", [("pass_cmp", "CMP"), ("pass_att", "ATT"), ("pass_yd", "YDS"), ("pass_td", "TD"), ("pass_int", "INT")]),
    ("Rushing", [("rush_att", "CAR"), ("rush_yd", "YDS"), ("rush_td", "TD")]),
    ("Receiving", [("rec", "REC"), ("rec_tgt", "TGT"), ("rec_yd", "YDS"), ("rec_td", "TD")]),
    ("Kicking", [("fgm", "FGM"), ("fga", "FGA"), ("xpm", "XPM"), ("xpa", "XPA"), ("kick_pts", "PTS")]),
    ("Defense", [("def_sack", "SACK"), ("def_int", "INT"), ("def_fum_rec", "FR"), ("def_td", "TD"), ("def_st_td", "ST TD")]),
]


def get_sleeper_week_player_stats(season, week):
    """Fetch Sleeper's weekly NFL player-stat feed in one request.

    This is a different service from the documented league matchup endpoint.
    It returns per-player NFL stats and is the primary stats source because the
    bot already has reliable connectivity to Sleeper on Railway.
    """
    url = (
        f"https://api.sleeper.com/stats/nfl/{int(season)}/{int(week)}"
        "?season_type=regular"
    )
    data = get_json(url)
    ...
    if isinstance(data, list):
        ...
        # Be tolerant of either a player-id keyed object or a wrapped list.
        ...
        raise RuntimeError(
            f"Sleeper stats returned unexpected payload type: {type(data).__name__}"
        )

    return rows


def sleeper_stat_row_player_id(row):
...
def sleeper_stat_row_stats(row):
    if not isinstance(row, dict):
        return {}
    stats = row.get("stats")
    if isinstance(stats, dict):
        return stats
    # Some variants flatten stats into the row. Keep only numeric fields.
    ignored = {"player_id", "player", "team", "week", "season", "season_type", "date", "opponent"}
    return {
        k: v for k, v in row.items()
        if k not in ignored and isinstance(v, (int, float))
    }


def calculate_league_fantasy_points(stats, scoring_settings):
    """Apply the league's Sleeper scoring settings directly to raw stats."""
    ...
    for stat_key, multiplier in scoring_settings.items():
        ...
        if not isinstance(multiplier, (int, float)):
            continue
        total += float(raw) * float(multiplier)
    return round(total, 2)


def format_sleeper_stat_lines(stats, fantasy_points=None):
    lines = []
    if fantasy_points is not None:
        lines.append(f"Fantasy Points: {float(fantasy_points):.2f}")

    used = set()
    for group_name, fields in SLEEPER_STAT_GROUPS:
        pieces = []
        for key, label in fields:
            ...
            used.add(key)
        if pieces:
            lines.append(f"{group_name}: " + ", ".join(pieces))

    # Make sure productive IDP/return/etc. performances are not hidden just
    # because their exact stat key is not in the compact display map.
    if len(lines) <= (1 if fantasy_points is not None else 0):
        extras = []
        for key, value in sorted(stats.items()):
            if key in used or key.startswith("pts_"):
                continue
            ...
            if len(extras) >= 6:
                break
        if extras:
            lines.append("Stats: " + ", ".join(extras))

    return lines

async def refresh_player_stats_pipeline(post_detailed=True):
    """Refresh the weekly player-stat feed primarily from Sleeper player stats.

    Sleeper supplies roster ownership and the weekly per-player NFL stat feed.
    League fantasy points are calculated from the league's own scoring_settings,
    so this path no longer depends on ESPN returning a scoreboard on Railway.
    ESPN remains elsewhere in the bot for NFL game/news features, but it is not
    required for this weekly fantasy player-stat feed.
    """
    diagnostics = {
        "week": None,
        "season": None,
        "sleeper_stat_rows": 0,
        "rostered_stat_rows": 0,
        "players_with_stats": 0,
        "cached_week_players": 0,
        "cumulative_synced": False,
        "errors": [],
        # Legacy keys retained so older code/log references cannot crash.
        "games_seen": 0,
        "completed_games": 0,
        "games_with_rostered_stats": 0,
        "matched_players": 0,
        "detailed_posts_sent": 0,
    }
    ...
    nfl_state = get_json("https://api.sleeper.app/v1/state/nfl")
    current_week = int(nfl_state.get("week", 1) or 1)
    current_season = int(nfl_state.get("season", datetime.now(central).year) or datetime.now(central).year)
    diagnostics["week"] = str(current_week)
    diagnostics["season"] = current_season
    ...
    if not isinstance(rosters_data, list) or not isinstance(users, list):
        raise RuntimeError("Sleeper rosters/users returned an unexpected payload.")

    scoring_settings = (
        league_data.get("scoring_settings", {})
        if isinstance(league_data, dict) else {}
    )
    ...
    # Build direct player-id ownership maps. No name matching is needed because
    # Sleeper's stats feed and Sleeper rosters use the same player IDs.
    pid_to_team = {}
    pid_to_position = {}
    rostered_pids = set()
    for roster in rosters_data:
        ...
        for raw_pid in roster.get("players", []) or []:
            ...
            if not pid or pid == "0":
                continue
            rostered_pids.add(pid)
            pid_to_team[pid] = team_name
            pid_to_position[pid] = (players_cache.get(pid, {}) or {}).get("position", "")

    try:
        stat_rows = get_sleeper_week_player_stats(current_season, current_week)
    except Exception as e:
        msg = f"Sleeper weekly stats fetch failed: {type(e).__name__}: {e}"
        diagnostics["errors"].append(msg)
        print(msg)
        stat_rows = []

    diagnostics["sleeper_stat_rows"] = len(stat_rows)
    week_key = str(current_week)
    week_data = week_player_stats.setdefault(week_key, {"teams": {}})
    week_data["teams"] = {}

    for row in stat_rows:
        pid = sleeper_stat_row_player_id(row)
        if not pid or pid not in rostered_pids:
            continue
        diagnostics["rostered_stat_rows"] += 1

        stats = sleeper_stat_row_stats(row)
        if not stats:
            continue

        # A row can contain only metadata/zero values for a player who has not
        # played. Ignore it until Sleeper reports a real stat or scoring value.
        has_activity = any(
            isinstance(v, (int, float)) and v != 0
            for k, v in stats.items()
            if not str(k).startswith("pts_")
        )
        pts_candidates = [stats.get("pts_ppr"), stats.get("pts_half_ppr"), stats.get("pts_std")]
        has_points = any(isinstance(v, (int, float)) and v != 0 for v in pts_candidates)
        if not has_activity and not has_points:
            continue

        fantasy_points = calculate_league_fantasy_points(stats, scoring_settings)
        lines = format_sleeper_stat_lines(stats, fantasy_points)
        if not lines:
            continue

        fantasy_team = pid_to_team.get(pid, "Unknown House")
        ...
        week_data["teams"].setdefault(fantasy_team, {})[pid] = entry
        diagnostics["players_with_stats"] += 1

    save_week_player_stats(week_player_stats)
    diagnostics["matched_players"] = diagnostics["players_with_stats"]
    diagnostics["cached_week_players"] = diagnostics["players_with_stats"]

    diagnostics["cumulative_synced"] = await sync_weekly_stats_feed(
        week_key, rosters_data, user_names, week_data
    )

    print(
        "PLAYER STATS PIPELINE COMPLETE: "
        f"season={current_season}; week={current_week}; "
        f"Sleeper rows={diagnostics['sleeper_stat_rows']}; "
        f"rostered rows={diagnostics['rostered_stat_rows']}; "
        f"players listed={diagnostics['players_with_stats']}; "
        f"synced={diagnostics['cumulative_synced']}; "
...
async def player_stats_checker():
    try:
        await refresh_player_stats_pipeline(post_detailed=True)
    except Exception as e:
        print(f"Player stats checker error: {type(e).__name__}: {e}")


@player_stats_checker.before_loop
async def before_player_stats_checker():
    await bot.wait_until_ready()


@bot.command(name="statstest")
async def statstest(ctx):
    """Run the exact production player-stats pipeline and report diagnostics."""
    try:
        result = await refresh_player_stats_pipeline(post_detailed=False)
        error_preview = ""
        if result["errors"]:
            error_preview = "\nErrors: " + " | ".join(result["errors"][:3])

        await ctx.send(
            "Player stats diagnostic complete.\n"
            f"Week: {result['week']}\n"
            f"Season: {result['season']}\n"
            f"Sleeper weekly stat rows returned: {result['sleeper_stat_rows']}\n"
            f"Rows belonging to your fantasy rosters: {result['rostered_stat_rows']}\n"
            f"Rostered players with actual stats: {result['players_with_stats']}\n"
            f"Players cached in weekly master feed: {result['cached_week_players']}\n"
            f"Weekly feed synced: {result['cumulative_synced']}"
            f"{error_preview}"
        )
    except Exception as e:
        await ctx.send(
            f"Player stats diagnostic failed: {type(e).__name__}: {e}"
        )
        print(f"!statstest error: {type(e).__name__}: {e}")
