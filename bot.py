...
def get_week_mvp(week):
    ...
    for m in matchups:
        ...
        # Sleeper's documented matchup response does not guarantee
        # per-player ``players_points``.  If it is absent, skip this roster
        # instead of incorrectly treating every starter as 0.00 and naming
        # the first starter MVP.
...
async def sync_weekly_stats_feed(current_week, rosters_data, user_names, week_data=None):
    """Edit the clean weekly feed from the SAME completed-player stats used by
    the detailed Stats channel.

    Important: Sleeper's documented matchup endpoint is team-level and should
    not be treated as a reliable source of per-player ``players_points``.  The
    detailed channel already has a working ESPN box-score parser that maps each
    completed NFL player back to a Sleeper roster.  ``week_data`` is populated
    from those successful matches, so using it here guarantees both channels
    are driven by the same completed-player records.
    """
    channel = bot.get_channel(ROSTER_STATS_CHANNEL_ID)
    if channel is None:
        try:
    ...
    print(
        ...
        f"{player_count} completed rostered player(s) cached from box scores."
    )
...
async def on_ready():
    ...
    try:
        ...
        print(
            ...
            f"finals={startup_stats.get('completed_games')}, "
            f"matched={startup_stats.get('matched_players')}, "
            f"cached={startup_stats.get('cached_week_players')}, "
...
async def refresh_player_stats_pipeline(post_detailed=True):
    """Refresh both NFL player-stat outputs from one authoritative pipeline.

    Source of individual box-score statistics: ESPN completed-game summaries.
    Source of fantasy roster ownership/team names: Sleeper rosters + users.

    The cumulative no-emoji feed is rebuilt from the exact same successfully
    matched player records used by the detailed Stats channel.  We deliberately
    do not depend on Sleeper matchup ``players_points`` because that field is
    not part of Sleeper's documented matchup response.
    """
    diagnostics = {
        "week": None,
        "games_seen": 0,
    ...
    # Player metadata is required for ESPN-ID and name matching.  on_ready()
    # normally fills this once, but reload here if startup fetching failed.
    if not players_cache:
        load_players_cache()

    state_url = "https://api.sleeper.app/v1/state/nfl"
    nfl_state = get_json(state_url)
    current_week = str(int(nfl_state.get("week", 1) or 1))
    diagnostics["week"] = current_week
    ...
    print(
        ...
        f"Week {current_week}, {len(rosters_data)} rosters, "
        f"{len(name_to_pid)} normalized roster names, "
        f"{len(espn_to_pid)} ESPN-ID mappings."
    )

    # Seven days safely covers Thursday/Monday games plus restarts later in the
    # week and allows the cumulative feed to rebuild from completed games.
    games = get_recent_nfl_games(days_back=7, week=int(current_week))
    diagnostics["games_seen"] = len(games)

    week_data = week_player_stats.setdefault(current_week, {"teams": {}})
    week_data.setdefault("teams", {})
    changed = False
    ...
    for game in games:
        try:
            ...
            diagnostics["completed_games"] += 1
            game_id = str(game.get("id") or "").strip()
            if not game_id:
                diagnostics["errors"].append("Completed ESPN event had no game ID")
                continue
            ...
            for competitor in competitors:
                abbr = (competitor.get("team", {}) or {}).get("abbreviation", "")
                if competitor.get("homeAway") == "away" and abbr:
                    away_team = abbr
                elif competitor.get("homeAway") == "home" and abbr:
                    home_team = abbr

            try:
                matched = extract_game_player_stats(
                    game_id, name_to_pid, espn_to_pid
                )
            except Exception as e:
                msg = f"ESPN box-score parse failed for {game_id}: {e}"
            ...
            if not matched:
                print(
                    f"PLAYER STATS PIPELINE: {away_team} @ {home_team} "
                    f"({game_id}) is final but matched 0 rostered players."
                )
                continue

            diagnostics["games_with_rostered_stats"] += 1
            diagnostics["matched_players"] += len(matched)
            by_team = {}

            for raw_pid, lines in matched.items():
                pid = str(raw_pid)
                fantasy_team = pid_to_team.get(raw_pid) or pid_to_team.get(pid)
                if not fantasy_team:
                    fantasy_team = "Unknown House"

                position = (
                    pid_to_position.get(raw_pid)
                    or pid_to_position.get(pid)
                    or (players_cache.get(pid, {}) or {}).get("position", "")
                )
                ...
                player_entry = {
                    ...
                    "lines": clean_lines,
                    "game_id": game_id,
                    "nfl_game": f"{away_team} @ {home_team}",
                }

                team_entry = week_data["teams"].setdefault(fantasy_team, {})
                old = team_entry.get(pid)
                if old != player_entry:
                    team_entry[pid] = player_entry
                    changed = True

                by_team.setdefault(fantasy_team, []).append(player_entry)

            if not by_team:
                continue

            # Detailed channel: one announcement per completed NFL game.
            if post_detailed and game_id not in processed_stat_events:
                message = f"🏈 **GAME FINAL: {away_team} @ {home_team}** 🏈\n\n"
                for fantasy_team, player_rows in by_team.items():
                    message += f"🏰 **{fantasy_team}**\n"
                    for player in player_rows:
                        pos = player.get("position", "")
                        pos_text = f" ({pos})" if pos else ""
                        message += f"**{player['name']}**{pos_text}\n"
                        message += "\n".join(player["lines"]) + "\n"
                    message += "\n"
                ...
                while len(message) > 1900:
                    ...
                    if split_at <= 0:
                        split_at = 1900
                    if not await send_player_stats_message(message[:split_at]):
                        detailed_ok = False
                        break
                    message = message[split_at:].lstrip()

                if detailed_ok and message:
                    detailed_ok = await send_player_stats_message(message)

                if detailed_ok:
                    processed_stat_events.add(game_id)
                    save_id_set(PROCESSED_STAT_EVENTS_FILE, processed_stat_events)
                    diagnostics["detailed_posts_sent"] += 1

        except Exception as e:
            # One bad game must never prevent later finals from being processed.
            game_id = str(game.get("id") or "unknown")
            msg = f"Player stats game-level error for {game_id}: {e}"
            diagnostics["errors"].append(msg)
            print(msg)
    ...
    # Always refresh the clean weekly master feed after scanning every final.
    # This also lets it reconstruct itself after a restart or deleted local ID file.
    ...
    cached_players = sum(
        len(players or {})
        for players in (week_data.get("teams", {}) or {}).values()
    )
    print(
        "PLAYER STATS PIPELINE COMPLETE: "
        f"Week {current_week}; ESPN games={diagnostics['games_seen']}; "
        f"finals={diagnostics['completed_games']}; "
        f"finals_with_rostered_stats={diagnostics['games_with_rostered_stats']}; "
        f"matched_this_scan={diagnostics['matched_players']}; "
        f"cached_week_players={cached_players}; "
        f"cumulative_synced={diagnostics['cumulative_synced']}; "
        f"errors={len(diagnostics['errors'])}."
    )

    diagnostics["cached_week_players"] = cached_players
    return diagnostics
...
async def statstest(ctx):
    ...
    try:
        ...
        await ctx.send(
            ...
            f"Week: {result['week']}\n"
            f"ESPN games scanned: {result['games_seen']}\n"
            f"Completed games found: {result['completed_games']}\n"
            f"Completed games with rostered stats: {result['games_with_rostered_stats']}\n"
            f"Rostered players matched this scan: {result['matched_players']}\n"
