...
# --- Persisted Discord message IDs for each team's roster post. ---
# --- This lets the bot edit one permanent roster post per team instead ---
# --- of creating a new message every time a roster changes. ---
...
def format_roster_post(roster, team_name):
    """Build the permanent Discord roster post for one fantasy team."""
    ...
    if starter_names:
        message += (
            "**Starters**\n"
            + "\n".join(f"• {name}" for name in starter_names)
            + "\n\n"
        )
    else:
        message += "**Starters**\n• None\n\n"
    ...
    if bench_names:
        message += (
            "**Bench**\n"
            + "\n".join(f"• {name}" for name in bench_names)
        )
    else:
        message += "**Bench**\n• None"
...
async def sync_roster_post(roster, team_name):
    """Edit a team's existing roster post, or create it if it doesn't exist."""
    channel = bot.get_channel(ROSTERS_CHANNEL_ID)

    if channel is None:
        print(f"Could not find Discord channel: {ROSTERS_CHANNEL_ID}")
        return False

    roster_id = roster.get("roster_id")
    key = str(roster_id)
    content = format_roster_post(roster, team_name)
    message_id = roster_message_ids.get(key)
    ...
    if message_id:
        try:
            ...
            # The saved roster post was deleted. Create a replacement below.
            roster_message_ids.pop(key, None)
        except discord.Forbidden:
            print(f"Missing permission to edit roster post for {team_name}.")
            return False
        except discord.HTTPException as e:
            print(f"Could not edit roster post for {team_name}: {e}")
            return False

    try:
        message = await channel.send(content)
        roster_message_ids[key] = str(message.id)
        save_roster_message_ids(roster_message_ids)
        return True
    except discord.HTTPException as e:
        print(f"Could not create roster post for {team_name}: {e}")
        return False


async def sync_all_roster_posts():
    """Synchronize every team's permanent roster post with Sleeper."""
    rosters_url = (
        f"https://api.sleeper.app/v1/league/"
        f"{LEAGUE_ID}/rosters"
    )
    ...
    for roster in rosters_data:
        ...
        await sync_roster_post(roster, team_name)
...
async def rosters(ctx):
    try:
        ...
        # Refresh snapshots too, so the automatic checker starts from the
        # same roster state that was just posted.
        for roster in rosters_data:
        ...
        await ctx.send(
            "✅ The permanent roster posts have been synced with Sleeper."
        )
...
async def roster_checker():
    try:
        ...
        for roster in rosters_data:
            ...
            # Migrate snapshots created by the older version, which stored
            # only a plain list of player IDs.
            ...
            # A team also needs syncing if its permanent Discord post has not
            # been created yet, even when the Sleeper roster itself is unchanged.
            post_missing = key not in roster_message_ids

            if roster_changed or post_missing:
                await sync_roster_post(roster, team_name)
                changed = True
