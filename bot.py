...
def update_weekly_scores_from_sleeper(current_week, rosters_data, user_names, games, week_data):
    """
    Populate the cumulative weekly feed from Sleeper's own matchup player scores.

    This avoids ESPN-to-Sleeper name matching entirely. Sleeper matchup data is
    already keyed by the exact player IDs used on league rosters. Only players
    whose NFL team has a completed game are written to the cumulative feed.
    """
    ...
    if changed:
        ...
        print(
            ...
            f"for completed NFL teams: {', '.join(sorted(completed_teams))}"
        )
...
async def player_stats_checker():
    try:
        ...
        # Populate the cumulative data-feed channel from Sleeper's own matchup
        # player scores. This uses exact Sleeper player IDs and therefore does
        # not depend on ESPN player-name matching. Only NFL teams whose games
        # are final are included.
