...
def update_weekly_scores_from_sleeper(
    ...
    """Populate the cumulative weekly feed from Sleeper matchup scores.

    Sleeper's ``players_points`` is the source of truth for fantasy points.
    To decide which players belong in the completed-game feed, prefer player IDs
    confirmed by ESPN's completed box scores. This avoids relying on Sleeper's
    cached ``player.team`` metadata, which can be stale or missing and previously
    caused finished-game players to be filtered out of the weekly post.
    """
    completed_player_ids = {str(pid) for pid in (completed_player_ids or set())}
    completed_teams = get_completed_nfl_teams(games)

    if not completed_player_ids and not completed_teams:
        return False
    ...
    for matchup in matchups or []:
        ...
        for pid, points in player_points.items():
            ...
            # Primary path: ESPN's completed box score confirmed this exact
            # Sleeper player ID participated in a finished game.
            if completed_player_ids:
                if pid not in completed_player_ids:
                    continue
            else:
                # Fallback for temporary ESPN box-score failures. This preserves
                # the older team-abbreviation behavior, but it is no longer the
                # main gate for the clean weekly feed.
                nfl_team = normalize_nfl_team_abbr(player_info.get("team"))
                if not nfl_team or nfl_team not in completed_teams:
                    continue
    ...
    if changed:
        ...
        print(
            f"Updated Week {current_week} cumulative stats from Sleeper "
            f"for {len(completed_player_ids)} confirmed completed-game player(s)."
        )
