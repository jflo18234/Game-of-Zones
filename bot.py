...
async def sync_weekly_stats_feed(current_week, rosters_data, user_names, week_data=None):
    """Rebuild and edit the clean weekly feed directly from Sleeper."""
    channel = bot.get_channel(ROSTER_STATS_CHANNEL_ID)
    if channel is None:
        try:
            channel = await bot.fetch_channel(ROSTER_STATS_CHANNEL_ID)
        except discord.HTTPException as e:
            print(f"Could not find cumulative NFL player stats channel {ROSTER_STATS_CHANNEL_ID}: {e}")
            return False
    ...
    try:
        ...
        print(f"Could not load Sleeper Week {current_week} matchup scores: {e}")
        return False

    if not isinstance(matchups, list):
        print(f"Sleeper Week {current_week} matchup payload was not a list.")
        return False

    chunks = build_weekly_stats_chunks_from_sleeper(
        current_week, rosters_data, user_names, matchups
    )
    ...
    # from the channel so a restart does not create another duplicate feed.
    if not ids:
        try:
    ...
    print(f"Synced Week {current_week} Sleeper player scores across {len(synced_ids)} fixed Discord message(s).")
    return True
