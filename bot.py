...
async def daily_raven():
    """Post one Daily Raven every calendar day, even on quiet news days."""
    try:
        ...
        if articles:
            message = (
                ...
                "The ravens have returned from across the realm with "
                "today's NFL and fantasy-relevant whispers.\n\n"
            )

            for article in articles[:5]:
                headline = article.get("headline", "Untitled NFL report")
                ...
                if description:
                    # Keep the Raven readable and safely under Discord's limit.
                    clean_description = description.strip()
                    if len(clean_description) > 240:
                        clean_description = clean_description[:237].rstrip() + "..."
            ...
            message += "🐦‍⬛ *The realm watches. Set your lineups accordingly.*"
        else:
            quiet_messages = [
                (
                    "🦅 **GAME OF ZONES — THE DAILY RAVEN** 🦅\n\n"
                    "No great news crosses the realm today. Even Varys has "
                    "nothing to whisper — which is frankly suspicious.\n\n"
                    "🐦‍⬛ *Check your lineups anyway. Winter is always coming.*"
                ),
                (
                    "🦅 **GAME OF ZONES — THE DAILY RAVEN** 🦅\n\n"
                    "The ravens returned with empty scrolls today. No major "
                    "NFL or fantasy decrees to report.\n\n"
                    "⚔️ *Enjoy the peace. It never lasts in this league.*"
                ),
                (
                    "🦅 **GAME OF ZONES — THE DAILY RAVEN** 🦅\n\n"
                    "The realm is unusually quiet. No major NFL news has "
                    "reached the maesters today.\n\n"
                    "🍷 *Tyrion recommends using the downtime to check the waiver wire.*"
                ),
            ]
            message = quiet_messages[today.toordinal() % len(quiet_messages)]
        ...
        print(f"Daily Raven posted for {today_key}.")
...
async def player_stats_checker():
    try:
        ...
        # Keep the dedicated no-emoji weekly data feed present and current even
        # after a restart, using whatever stats have already been persisted.
        ...
        for game in games:
            ...
            if matched:
                ...
                # Rewrite the same cumulative weekly data-feed messages so the
                # new game's players appear without creating additional posts.
                await sync_weekly_stats_feed(
                    current_week, rosters_data, user_names, week_data
                )

            # Only mark the game processed after rostered-player stats were
            # actually found and posted.  If ESPN's box score is temporarily
            # incomplete, the checker will retry on the next pass instead of
            # permanently losing that game's stats.
            if matched:
